...
import { createAmendmentService } from './amendment.service'
...
export const createContractService = (
  ...
  const amendmentService = createAmendmentService(contractRepository, accessRepo, requireAdmin)
  ...
  return {
    ...
    requestSignature: async (actor: Actor, projectId: string, meta: RequestMeta) => {
      ...
      return db.transaction(async (tx) => {
        ...
        await createAuditRepository(tx).createAuditLog({
          actorSub: actor.sub,
          action: 'project_contract_signature_requested',
          resourceType: 'project_contract',
          resourceId: contract.id,
          ipAddress: meta.ipAddress,
          userAgent: meta.userAgent,
    ...
    listAmendments: amendmentService.listAmendments,
    saveAmendmentDraft: amendmentService.saveAmendmentDraft,
    requestClientAmendment: amendmentService.requestClientAmendment,
    requestAmendmentSignature: amendmentService.requestAmendmentSignature,
    signAmendment: amendmentService.signAmendment,
  }
}
